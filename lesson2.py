...
class Human:
    ...
    def __str__(self):  # метод, що спрацьовує при перетворенні на str
        return f"Об'єкт класу людина з ім'ям {self.name}"

    def __int__(self):  # при перетворенні на число, об'єкт буде повертати свій вік
        return self.age
...
class Auto:
    def __init__(self, brand, max_passengers=5):
        ...
        self.passengers = []  # [] - список пасажирів
...
bob = Human("Боб", 17, 180)  # створення екземпляру класу
bob.say_hi()
